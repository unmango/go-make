...
| CRLF line endings                 | `target: prereq\r\n`                            |                    |                    |                    | no text is dropped, prints with LF and an extra blank line per line, [#158](https://github.com/unmango/go-make/issues/158)    |
...
| semicolon delimited recipes       | `target: ;recipe text`                          | :white_check_mark: | :white_check_mark: |                    | `target: prereq;recipe` still scans `prereq;recipe` as a pre-requisite, [#112](https://github.com/unmango/go-make/issues/112) |
| custom `.RECIPEPREFIX`            | `.RECIPEPREFIX = >\n>recipe`                    |                    |                    |                    | the prefix is fixed to `TAB`, [#178](https://github.com/unmango/go-make/issues/178)                                           |
| **variables**                     |                                                 |                    |                    |                    |                                                                                                                               |
| assignment operators              | `=`, `:=`, `::=`, `:::=`, `?=`, `!=`            | :white_check_mark: | :white_check_mark: |                    |                                                                                                                               |
| append assignment                 | `VAR += x`                                      | :white_check_mark: | :white_check_mark: |                    | the unspaced `VAR+=x` is still stored as a `BadObj`, see the row below                                                        |
...
| unspaced `=`, `?=`, `!=`, `+=`    | `VAR=x`, `VAR?=x`, `VAR!=x`, `VAR+=x`           |     :warning:      |     :warning:      |                    | scanned as one `TEXT` token and stored as a `BadObj`, [#112](https://github.com/unmango/go-make/issues/112)                   |
...
| inside a recipe block             | `target:\nifdef V\n\techo x\nendif`             |     :warning:      |     :warning:      |                    | the directive is parsed, the recipe line is stored as a `BadObj`, [#139](https://github.com/unmango/go-make/issues/139)       |
| **other directives**              |                                                 |                    |                    |                    |                                                                                                                               |
