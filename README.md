...
| escaped `#`                       | `target: foo\#bar`                              | :white_check_mark: | :white_check_mark: |                    | kept verbatim, no unescaping is performed, and `foo\\#bar` escapes the backslash so the `#` starts a comment                  |
...
| rule comments                     | `target: prereq # comment text`                 | :white_check_mark: | :white_check_mark: |                    | recorded on `Rule.Comment`, which shares the target line rather than starting one                                             |
| variable comments                 | `VAR = val # comment text`                      | :white_check_mark: | :white_check_mark: |                    | recorded on `Variable.Comment`                                                                                                |
| comments after a word             | `target: prereq# comment text`                  | :white_check_mark: | :white_check_mark: |                    | make removes the comment before reading the line, so the prerequisite is `prereq`                                             |
| conditional comments              | `ifeq (a,b) # comment text`                     | :white_check_mark: | :white_check_mark: |                    | recorded on the directive, so `ifeq`, `ifdef`, a bare `else`, and `endif` each keep the comment ending their own line         |
| `else if` comments                | `else ifeq (a,b) # comment text`                | :white_check_mark: | :white_check_mark: |                    | recorded on the condition rather than on `ElseBlock.Comment`, which holds the comment ending a bare `else`                    |
| conditional with no condition     | `ifeq # comment text`                           |                    |                    |                    | parse error, and the comment is consumed with the token the parser expected                                                   |
| comments in a `define` body       | `define B\ntext # text\nendef`                  |     :warning:      |     :warning:      |                    | not comments to make, every line of the body is a `BadObj` holding the text                                                   |
| **rules**                         |                                                 |                    |                    |                    |                                                                                                                               |
...
| definition blocks                 | `define FOO\nbody\nendef`                       | :white_check_mark: | :white_check_mark: |                    | one `DefineDir` holding the body as the literal lines make records, a nested `define` is counted the way make counts it       |
| definition block operators        | `define FOO :=`, `define FOO +=`                | :white_check_mark: | :white_check_mark: |                    | every assignment operator an ordinary assignment accepts                                                                      |
| `undefine`                        | `undefine VAR`                                  | :white_check_mark: | :white_check_mark: |                    | one `UndefineDir`, a directive naming several variables is a parse error and the line is kept as a `BadObj`                   |
