...
| append assignment                 | `VAR += x`                                      |     :warning:      |     :warning:      |                    | `+=` is not tokenized, the line is stored as a `BadObj`, [#137](https://github.com/unmango/go-make/issues/137)                |
...
| unspaced `=`, `?=`, `!=`, `+=`    | `VAR=x`, `VAR?=x`, `VAR!=x`                     |     :warning:      |     :warning:      |                    | scanned as one `TEXT` token and stored as a `BadObj`, [#112](https://github.com/unmango/go-make/issues/112)                   |
