...
var _ = Describe("Scan", func() {
	Describe("ScanTokens", func() {
		DescribeTable("Scanner",
			...
			Entry("semicolon written against the text on both sides",
				"target: a;recipe",
				[]string{"target", ":", " ", "a", ";", "recipe"},
			),
			Entry("pipe with no separating space",
				"target:|prereq", []string{"target", ":", "|", "prereq"},
			),
			Entry("pipe written against the text on both sides",
				"target: prereq|order",
				[]string{"target", ":", " ", "prereq", "|", "order"},
			),
			...
			Entry("unspaced recursive variable",
				"VAR=x", []string{"VAR", "=", "x"},
			),
			Entry("unspaced ifndef variable",
				"VAR?=x", []string{"VAR", "?=", "x"},
			),
			Entry("unspaced shell variable",
				"VAR!=x", []string{"VAR", "!=", "x"},
			),
			Entry("unspaced append variable",
				"VAR+=x", []string{"VAR", "+=", "x"},
			),
			Entry("assignment operator written in a value",
				"VAR = a=b", []string{"VAR", " ", "=", " ", "a", "=", "b"},
			),
			Entry("exclamation mark written in a value",
				"VAR = a!b", []string{"VAR", " ", "=", " ", "a", "!", "b"},
			),
			Entry("exclamation mark at the end of the input",
				"VAR = a!", []string{"VAR", " ", "=", " ", "a", "!"},
			),
			Entry("plus sign written without an equals sign",
				"VAR = a+b", []string{"VAR", " ", "=", " ", "a+b"},
			),
			Entry("question mark written without an equals sign",
				"VAR = a?b", []string{"VAR", " ", "=", " ", "a?b"},
			),
			Entry("assignment operator in a target line",
				"target: a=b", []string{"target", ":", " ", "a", "=", "b"},
			),
			Entry("info function",
			...
			Entry("reference written against the text before it",
				"prefix$(FOO)",
				[]string{"prefix", "$", "(", "FOO", ")"},
			),
			Entry("reference written against the text after it",
				"$(FOO)suffix",
				[]string{"$", "(", "FOO", ")", "suffix"},
			),
			Entry("reference written between text",
				"a$(FOO)b",
				[]string{"a", "$", "(", "FOO", ")", "b"},
			),
			Entry("braced reference written against the text before it",
				"prefix${FOO}",
				[]string{"prefix", "$", "{", "FOO", "}"},
			),
			Entry("escaped dollar written against the text around it",
				"pre$$post",
				[]string{"pre", "$", "$", "post"},
			),
			Entry("dollar sign at the end of the input",
				"a$", []string{"a", "$"},
			),
			Entry("ifeq directive",
				"ifeq (foo, bar)", []string{"ifeq", " ", "(", "foo", ",", " ", "bar", ")"},
			),
			Entry("ifeq directive with a reference in its first argument",
				"ifeq (x$(V),y)",
				[]string{"ifeq", " ", "(", "x", "$", "(", "V", ")", ",", "y", ")"},
			),
		...
		DescribeTable("assignments split across reads",
			...
			Entry("exclamation mark written as text",
				"AAA!b\n", []string{"AAA", "!", "b", "\n"},
			),
			func(text string, expected []string) {
